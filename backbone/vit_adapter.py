    ...
    def sum_adapter_param(self):
        for layer_idx in range(len(self.blocks)):
            if len(self.down_weight_sum[layer_idx]) > 0:
                self.down_weight_sum[layer_idx].append(
                    self.cur_adapter[layer_idx].down_proj.weight.data + self.down_weight_sum[layer_idx][-1])
                self.down_bias_sum[layer_idx].append(
                    self.cur_adapter[layer_idx].down_proj.bias.data + self.down_bias_sum[layer_idx][-1])
                self.up_weight_sum[layer_idx].append(
                    self.cur_adapter[layer_idx].up_proj.weight.data + self.up_weight_sum[layer_idx][-1])
                self.up_bias_sum[layer_idx].append(
                    self.cur_adapter[layer_idx].up_proj.bias.data + self.up_bias_sum[layer_idx][-1])
            else:
